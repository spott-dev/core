...
function buildFilter(filter = {}) {
  const { types, countryIds, adminDivision1Ids, adminDivision2Ids, coordinates } = filter;
  const filtersArray = [];
  ...
  if (isArray(countryIds) && !isEmpty(countryIds)) {
    const queries = buildTermQueries('country.id', countryIds);
    filtersArray.push(or(queries));
  }

  if (isArray(adminDivision1Ids) && !isEmpty(adminDivision1Ids)) {
    const queries = buildTermQueries('adminDivision1.id', adminDivision1Ids);
    filtersArray.push(or(queries));
  }

  if (isArray(adminDivision2Ids) && !isEmpty(adminDivision2Ids)) {
    const queries = buildTermQueries('adminDivision2.id', adminDivision2Ids);
    filtersArray.push(or(queries));
  }
