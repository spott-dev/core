...
async function discoverPlaceById({database, ip}) {
  ...
  const guessCity = async (ipInsights) => {
    const {country, city, coordinates} = ipInsights;
    const {name} = city;
    const types = ['CITY'];
    const countryIds = [country.iso2];
    const filter = { types, countryIds, coordinates };
    const options = { limit: 1, fields: ['id'] };

    const [indexedPlace] = await database.indexedPlaces.findByQueryAndFilter(name, filter, options);
    if (!indexedPlace) return null;

    return database.places.findById(indexedPlace.id);
  };
