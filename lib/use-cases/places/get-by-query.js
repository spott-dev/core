 ...
 */
async function getPlacesByQuery(params) {
  const {database, query, languages} = params;
  const filter = pick(params, FILTER_PARAMS);
  const options = {
    ...pick(params, OPTIONS_PARAMS),
    fields: ['id', 'score']
  };
  const indexedPlaces = await database.indexedPlaces.findByQueryAndFilter(query, filter, options);
