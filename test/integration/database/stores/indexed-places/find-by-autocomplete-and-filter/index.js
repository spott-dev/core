...
describe('Database | IndexedPlacesStore | .findByAutocompleteAndFilter', () => {
  ...
  it('should get multiple places by a query', async () => {
    ...
    const places = await database.indexedPlaces.findByAutocompleteAndFilter(query, filter);
  ...
  it.skip('should search a place by string misspelled', async () => {
    const query = 'Gadelajara';
    const filter = undefined;
  ...
  it('should autocomplete a place search', async () => {
    const query = 'Guad';
    const filter = undefined;
  ...
  it('should autocomplete all pieces of the string to be more specific', async () => {
    const query = 'Guad Spa';
    const filter = undefined;
  ...
  it('should support limit option', async () => {
    ...
    const places = await database.indexedPlaces.findByAutocompleteAndFilter(query, filter, options);
    expect(places).to.be.an('array');
  ...
  it('should get places of any type by default, returning most populated first', async () => {
    ...
    const places = await database.indexedPlaces.findByAutocompleteAndFilter(query, filter);
    expect(places).to.be.an('array');
    expect(places.length).to.be.equal(3);
